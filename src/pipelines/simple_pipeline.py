    ...
    def execute(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                trend_file = self.temp_manager.create_temp_file(
                    "trend_analysis", "trends", "json"
                )
            ...
            try:
                ...
                video_file = self.temp_manager.create_video_file(
                    "video_generation", "mp4", "raw"
                )
                ...
                if result_video and os.path.exists(result_video):
                    ...
                    if file_size > 0.1:  # File has content
                        logger.info(
                            f"Video generated successfully (despite error): {video_file} ({file_size:.1f} MB)"
                        )
                    else:
            ...
            try:
                if self.audio_generator:
                    ...
                    audio_file = self.temp_manager.create_audio_file(
            ...
            try:
                if audio_file and self.media_combiner and os.path.exists(audio_file):
                    ...
                    combined_file = self.temp_manager.create_video_file(
                        "media_combination", "mp4", "combined"
                    )
            ...
            try:
                if self.video_enhancer:
                    ...
                    if not os.path.exists(current_video):
                        logger.error(
                            f"Video file for enhancement not found: {current_video}"
                        )
                        self.temp_manager.mark_error()
                        return None

                    # Create enhanced file path using unified temp manager
                    enhanced_file = self.temp_manager.create_video_file(
                        "video_enhancement", "mp4", "enhanced"
                    )

                    # Prepare enhancement options
                    hashtags = (
                        trend_data.popular_hashtags[:8]
                        if trend_data
                        else ["fyp", "viral"]
                    )
                    options = {
                    ...
                    if enhanced_result and os.path.exists(enhanced_result):
                        ...
                        logger.warning(
                            "Video enhancement failed, using unenhanced video"
                        )
                        # Keep current_video as is
                else:
            ...
            try:
                ...
                final_path = os.path.join(
                    self.config["output_dir"], f"final_{timestamp}.mp4"
                )
                ...
                logger.info(
                    f"Temporary files: {stats['total_files']} files, {stats['total_size_mb']:.1f} MB"
                )
    ...
    def _publish_video(self, video_path: str, trend_data):
        ...
        for platform, publisher in self.publishers.items():
            try:
                ...
                hashtags = (
                    trend_data.popular_hashtags[:10]
                    if trend_data
                    else ["physics", "simulation", "viral", "fyp"]
                )
...
def create_simple_pipeline(
    output_dir: str = "videos", auto_publish: bool = False, video_duration: int = 60
) -> SimplePipeline:
    """
