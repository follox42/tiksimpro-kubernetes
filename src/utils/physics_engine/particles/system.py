...
class ParticleEmitter:
    ...
    def _create_particle(self) -> ParticleData:
        ...
        # Vitesse initiale
        angle = math.radians(
            random.uniform(self.particle_angle_min, self.particle_angle_max)
        )
    ...
    def _vary_color(
        self, base_color: Tuple[int, int, int], variance: float
    ) -> Tuple[int, int, int]:
        """Applique une variance à une couleur"""
        r, g, b = base_color

        r = max(
            0, min(255, r + random.randint(-int(variance * 255), int(variance * 255)))
        )
        g = max(
            0, min(255, g + random.randint(-int(variance * 255), int(variance * 255)))
        )
        b = max(
            0, min(255, b + random.randint(-int(variance * 255), int(variance * 255)))
        )
    ...
    def _update_particle(self, particle: ParticleData, dt: float):
        ...
        if particle.drag > 0 and particle.velocity.magnitude > 0:
            drag_force = (
                ...
                * -particle.velocity.magnitude_squared
                * particle.drag
            )
    ...
    def _update_particle_visuals(self, particle: ParticleData, life_progress: float):
        ...
        if self.size_over_life:
            particle.size = self.size_over_life(life_progress) * particle.metadata.get(
...
class EffectManager:
    ...
    def create_explosion_effect(
        ...
        emitter.size_over_life = lambda t: smooth_step(0.0, 0.3, t) * (
            1.0 - smooth_step(0.7, 1.0, t)
        )
    ...
    def create_collision_effect(
        self, position: Vector2D, normal: Vector2D, intensity: float = 1.0
    ):
    ...
    def add_screen_flash(
        self, intensity: float, color: Tuple[int, int, int] = (255, 255, 255)
    ):
