...
class PhysicsBody(ABC):
    ...
    def render_glow(self, screen: pygame.Surface):
        ...
        glow_surf = pygame.Surface(
            (self.glow_radius * 4, self.glow_radius * 4), pygame.SRCALPHA
        )
        ...
        # Blitter sur l'écran
        glow_pos = (
            self.position.x - self.glow_radius * 2,
...
class Circle(PhysicsBody):
    ...
    def __init__(
        self, position: Vector2D, radius: float, mass: float = 1.0, static: bool = False
    ):
    ...
    def render(self, screen: pygame.Surface):
        ...
        if self.outline_width > 0:
            pygame.draw.circle(
                screen, self.outline_color, pos, radius, self.outline_width
            )

    def _render_striped_circle(
        self, screen: pygame.Surface, pos: Tuple[int, int], radius: int
    ):
        ...
        for i in range(-radius, radius, stripe_width * 2):
            ...
            pygame.draw.rect(
                temp_surf, color, (i + radius, 0, stripe_width, radius * 2)
            )
...
class Segment(PhysicsBody):
    ...
    def _render_dashed_line(
        self, screen: pygame.Surface, start: tuple, end: tuple, thickness: int
    ):
        ...
        while current_pos < length:
            segment_length = min(
                self.dash_length if drawing else self.dash_gap, length - current_pos
            )

            if drawing:
                seg_start = (
                    start[0] + direction.x * current_pos,
                    start[1] + direction.y * current_pos,
                )
                seg_end = (
                    start[0] + direction.x * (current_pos + segment_length),
    ...
    def _render_flow_line(
        ...
        if flow_pos < length:
            ...
            if seg_end_pos > seg_start_pos:
                seg_start = (
                    start[0] + direction.x * seg_start_pos,
                    start[1] + direction.y * seg_start_pos,
                )
                seg_end = (
                    start[0] + direction.x * seg_end_pos,
...
class Ring(PhysicsBody):
    ...
    def collision_with_circle(self, circle_pos: Vector2D, circle_radius: float) -> dict:
        ...
        if distance < self.inner_radius + circle_radius:
            ...
            collision_info = {
                ...
                + (circle_pos - self.position).normalized * self.outer_radius,
            }
    ...
    def _render_segmented_ring(
        self, screen: pygame.Surface, center: tuple, color: tuple
    ):
