...
class TikTokAdsConnector:
    ...
    def __init__(
        self, cookies_file: str = "tiktok_ads_cookies.pkl", headless: bool = False
    ):
        ...
        if os.path.exists(self.cookies_file):
            try:
                ...
                if self._is_logged_in():
                    logger.info(
                        "Session TikTok ads restaurée avec succès via les cookies"
                    )
                    self.is_authenticated = True
                    return True
                else:
                    logger.info(
                        "Les cookies sauvegardés ne sont plus valides pour TikTok ads"
                    )
                    return False
    ...
    def _is_logged_in(self) -> bool:
        ...
        try:
            ...
            if (
                "tiktok.com/foryou" in current_url and "login" not in current_url
            ) or "https://ads.tiktok.com" in current_url:
                return True
            ...
            logo = self.driver.find_elements(
                By.XPATH, "//*[@id='tt4b-nav-guidance-4']/div/div[1]"
            )
            if logo:
                return False

            mobile_stats = self.driver.find_elements(
                By.XPATH, "//div[@class='TUXTooltip-reference']"
            )
            if mobile_stats:
                return False
    ...
    def authenticate(self) -> bool:
        ...
        if not self.driver:
            logger.info(
                "Initialisation du navigateur pour l'authentification TikTok..."
            )
            if not self._setup_browser():
        ...
        try:
            ...
            print("\n" + "=" * 80)
            print(
                "VEUILLEZ VOUS CONNECTER MANUELLEMENT À TIKTOK ADS DANS LA FENÊTRE DU NAVIGATEUR"
            )
            ...
            while time.time() - start_time < max_wait_time:
                ...
                print(
                    f"Vérification de connexion TikTok ads... URL: {self.driver.current_url}"
                )
