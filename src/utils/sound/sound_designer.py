...
class SoundDesignerGUI:
    def __init__(self):
        ...
        self.current_config = {
            ...
            "effects": {"reverb": 0.2, "compression": 0.7, "eq_boost": 1.0},
        }
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="🎵 Sound Designer Ultra-Complet - Tous Paramètres",
            font=("Arial", 16, "bold"),
        )
    ...
    def setup_complete_interface(self):
        ...
        # Nom du son
        ttk.Label(base_frame, text="Nom du son:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky=tk.W
        )
        self.name_var = tk.StringVar(value=self.current_config["name"])
        name_entry = ttk.Entry(
            base_frame, textvariable=self.name_var, font=("Arial", 9)
        )
        ...
        ttk.Label(base_frame, text="Type:").grid(
        ...
        ttk.Label(base_frame, text="Forme d'onde:").grid(
        ...
        self.freq_min_var = tk.DoubleVar(
            value=self.current_config["frequency_range"][0]
        )
        freq_min_scale = ttk.Scale(
            freq_vol_frame,
            from_=50,
            to=500,
            variable=self.freq_min_var,
        ...
        ttk.Label(freq_vol_frame, text="Aigu (Hz):").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.freq_max_var = tk.DoubleVar(
            value=self.current_config["frequency_range"][1]
        )
        freq_max_scale = ttk.Scale(
            freq_vol_frame,
            from_=200,
            to=2000,
            variable=self.freq_max_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        # Volume et durée
        ttk.Label(freq_vol_frame, text="Volume:").grid(
            row=1, column=0, sticky=tk.W, pady=(10, 0)
        )
        ...
        ttk.Label(freq_vol_frame, text="Durée (s):").grid(
            row=1, column=3, sticky=tk.W, padx=(20, 0), pady=(10, 0)
        )
        ...
        # === HARMONIQUES ===
        harm_frame = ttk.LabelFrame(
            parent, text="🎼 Harmoniques & Sous-Harmoniques", padding=10
        )
        ...
        harm2_scale = ttk.Scale(
            harm_frame,
            from_=0,
            to=1,
            variable=self.harm2_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(harm_frame, text="Harmonique 3:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.harm3_var = tk.DoubleVar(value=0.0)
        harm3_scale = ttk.Scale(
            harm_frame,
            from_=0,
            to=1,
            variable=self.harm3_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(harm_frame, text="Harmonique π:").grid(
            row=1, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.harm_pi_var = tk.DoubleVar(value=0.0)
        harm_pi_scale = ttk.Scale(
            harm_frame,
            from_=0,
            to=0.5,
            variable=self.harm_pi_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(harm_frame, text="Golden Ratio:").grid(
            row=1, column=3, sticky=tk.W, padx=(20, 0), pady=(10, 0)
        )
        self.harm_golden_var = tk.DoubleVar(value=0.0)
        harm_golden_scale = ttk.Scale(
            harm_frame,
            from_=0,
            to=0.5,
            variable=self.harm_golden_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        harm_golden_scale.grid(
            row=1, column=4, sticky=tk.EW, padx=(10, 0), pady=(10, 0)
        )
        ...
        self.attack_var = tk.DoubleVar(
            value=self.current_config["envelope"]["attack_ms"]
        )
        ...
        ttk.Label(env_frame, text="Release (ms):").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.release_var = tk.DoubleVar(
            value=self.current_config["envelope"]["release_ms"]
        )
        ...
        fm_rate_scale = ttk.Scale(
            mod_frame,
            from_=0,
            to=20,
            variable=self.fm_rate_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(mod_frame, text="FM Depth:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.fm_depth_var = tk.DoubleVar(value=0.0)
        fm_depth_scale = ttk.Scale(
            mod_frame,
            from_=0,
            to=0.5,
            variable=self.fm_depth_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        # AM
        ttk.Label(mod_frame, text="AM Rate (Hz):").grid(
            row=1, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.am_rate_var = tk.DoubleVar(value=0.0)
        am_rate_scale = ttk.Scale(
            mod_frame,
            from_=0,
            to=15,
            variable=self.am_rate_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(mod_frame, text="AM Depth:").grid(
            row=1, column=3, sticky=tk.W, padx=(20, 0), pady=(10, 0)
        )
        self.am_depth_var = tk.DoubleVar(value=0.0)
        am_depth_scale = ttk.Scale(
            mod_frame,
            from_=0,
            to=0.8,
            variable=self.am_depth_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(turb_frame, text="Flutter:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.flutter_var = tk.DoubleVar(value=0.0)
        flutter_scale = ttk.Scale(
            turb_frame,
            from_=0,
            to=0.1,
            variable=self.flutter_var,
            orient=tk.HORIZONTAL,
            command=self.on_config_change,
        )
        ...
        ttk.Label(filt_frame, text="Résonance:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        self.resonance_var = tk.DoubleVar(value=0.0)
        resonance_scale = ttk.Scale(
            filt_frame,
            from_=0,
            to=1.5,
        ...
        ttk.Label(fx_frame, text="Delay:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 0)
        )
        ...
        ttk.Label(fx_frame, text="Chorus:").grid(
            row=1, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.chorus_var = tk.DoubleVar(value=0.0)
        chorus_scale = ttk.Scale(
            fx_frame,
            from_=0,
            to=1,
        ...
        ttk.Label(fx_frame, text="Distortion:").grid(
            row=1, column=3, sticky=tk.W, padx=(20, 0), pady=(10, 0)
        )
        self.distortion_var = tk.DoubleVar(value=0.0)
        distortion_scale = ttk.Scale(
            fx_frame,
            from_=0,
            to=1,
    ...
    def setup_presets_panel(self, parent):
        ...
        save_btn = ttk.Button(
            buttons_frame, text="💾 Sauvegarder", command=self.save_current_config
        )
        save_btn.pack(fill=tk.X, pady=(0, 5))

        delete_btn = ttk.Button(
            buttons_frame, text="🗑️ Supprimer", command=self.delete_preset
        )
        delete_btn.pack(fill=tk.X, pady=(0, 5))

        export_btn = ttk.Button(
            buttons_frame, text="📤 Exporter JSON", command=self.export_configs
        )
        export_btn.pack(fill=tk.X, pady=(0, 5))

        import_btn = ttk.Button(
            buttons_frame, text="📥 Importer JSON", command=self.import_configs
        )
    ...
    def setup_playback_controls(self, parent):
        ...
        test_btn = ttk.Button(
            buttons_frame,
            text="🎵 Tester le Son Ultra-Complet",
            command=self.test_advanced_sound,
        )
        test_btn.pack(side=tk.LEFT, padx=(0, 5))

        random_btn = ttk.Button(
            buttons_frame,
            text="🎲 CONFIGURATION FOLLE",
            command=self.generate_random_sound,
        )
        random_btn.pack(side=tk.LEFT, padx=(0, 10))

        # Boutons MIDI
        midi_file_btn = ttk.Button(
            buttons_frame, text="📁 Choisir MIDI", command=self.select_midi_file
        )
        midi_file_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.midi_btn = ttk.Button(
            buttons_frame, text="🎼 Play MIDI", command=self.toggle_midi_playback
        )
        self.midi_btn.pack(side=tk.LEFT)

        # Label pour le fichier MIDI sélectionné
        self.midi_file_label = ttk.Label(
            playback_frame, text="Aucun fichier MIDI sélectionné", font=("Arial", 8)
        )
        ...
        status_label = ttk.Label(
            playback_frame, textvariable=self.status_var, font=("Arial", 9, "italic")
        )
        ...
        self.current_config.update(
            {
                ...
                "frequency_range": [
                    int(self.freq_min_var.get()),
                    int(self.freq_max_var.get()),
                ],
        ...
        def play_advanced():
            try:
                ...
                config["envelope"] = {
                    ...
                    "sustain_level": self.current_config["envelope"].get(
                        "sustain_level", 0.7
                    ),
                ...
                if hasattr(self, "fm_rate_var") and self.fm_rate_var.get() > 0:
                    config["modulation"] = {
                        ...
                        "fm_depth": getattr(
                            self, "fm_depth_var", tk.DoubleVar(value=0.1)
                        ).get(),
                    }
    ...
    def generate_configured_sound(self, frequency):
        ...
        if hasattr(self, "harm2_var") and self.harm2_var.get() > 0:
            harmonics.append(
                {"harmonic": 2, "amplitude": self.harm2_var.get(), "waveform": "sine"}
            )
        if hasattr(self, "harm3_var") and self.harm3_var.get() > 0:
            harmonics.append(
                {
        ...
        if hasattr(self, "harm_pi_var") and self.harm_pi_var.get() > 0:
            harmonics.append(
                {
                    "harmonic": 3.14159,
                    "amplitude": self.harm_pi_var.get(),
                    "waveform": "sine",
                }
            )
        if hasattr(self, "harm_golden_var") and self.harm_golden_var.get() > 0:
            harmonics.append(
                {
                    "harmonic": 1.618,
                    "amplitude": self.harm_golden_var.get(),
                    "waveform": "triangle",
                }
            )
        ...
        if hasattr(self, "reverb_var") and self.reverb_var.get() > 0:
            effects.append(
                {"type": "reverb", "room_size": 0.5, "wet_level": self.reverb_var.get()}
            )
        if hasattr(self, "delay_var") and self.delay_var.get() > 0:
            effects.append(
                {
            ...
            effects.append(
                {
                    "type": "distortion",
                    "drive": 5.0,
                    "wet_level": self.distortion_var.get(),
                }
            )
        if effects:
            config["effects"] = effects
            ...
            try:
                ...
                for msg in mid.play():
                    ...
                    if (
                        ...
                        and getattr(msg, "type", None) == "note_on"
    ...
    def save_current_config(self):
        ...
        if self.current_config["name"] in existing_names:
            if not messagebox.askyesno(
                "Remplacer",
                f"Le preset '{self.current_config['name']}' existe déjà. Remplacer?",
            ):
                return
            # Supprime l'ancien
            self.saved_configs = [
    ...
    def delete_preset(self):
        ...
        if selection:
            ...
            if messagebox.askyesno(
                "Supprimer", f"Supprimer le preset '{config_name}'?"
            ):
    ...
    def load_configs(self):
        ...
        try:
            if os.path.exists("sound_presets/custom_sounds.json"):
                with open(
                    "sound_presets/custom_sounds.json", "r", encoding="utf-8"
                ) as f:
