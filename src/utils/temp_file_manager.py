...
class TempFileManager:
    ...
    def create_temp_file(
        self, step_name: str, filename: str, extension: str = "", unique: bool = True
    ) -> Path:
        """
    ...
    def create_audio_file(
        self, step_name: str = "audio_generation", format: str = "wav"
    ) -> Path:
        """Create temporary audio file"""
        return self.create_temp_file(step_name, "audio", f".{format}")

    def create_video_file(
        self, step_name: str, format: str = "mp4", quality: str = "temp"
    ) -> Path:
    ...
    def cleanup_all(self, force: bool = False) -> None:
        ...
        try:
            ...
            logger.info(
                f"Cleanup completed: {file_count} files, {size_mb:.1f} MB freed"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for step_name in self.step_dirs:
            step_files = self.list_files(step_name)
            step_size = sum(f.stat().st_size for f in step_files if f.exists()) / (
                1024 * 1024
            )
...
def cleanup_all_temp_files(base_dir: str = "temp", max_age_hours: int = 1) -> None:
    ...
    try:
        ...
        if cleaned_count > 0:
            ...
            logger.info(
                f"Global cleanup: {cleaned_count} sessions, {size_mb:.1f} MB freed"
            )
