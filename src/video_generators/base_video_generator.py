...
class IVideoGenerator(ABC):
        ...
        try:
            ...
            if not self.headless_mode:
                ...
                pygame.display.set_caption(
                    f"{self.__class__.__name__} - TikSimPro [FAST MODE]"
                )
            ...
            logger.info(
                f"Pygame PERFORMANCE mode initialized: {self.width}x{self.height}"
            )
            logger.info(
                f"Headless mode: {self.headless_mode}, Fast mode: {self.fast_mode}"
            )
            return True
    ...
    def setup_ffmpeg_recording(self, use_gpu: bool = True) -> bool:
        ...
        try:
            ...
            encoder, preset, extra_args = self._get_best_encoder(
                False
            )  # Stable CPU encoder
    ...
    def _get_best_encoder(self, use_gpu: bool = False) -> Tuple[str, str, List[str]]:
        ...
        try:
            ...
            if use_gpu:
                ...
                if "h264_nvenc" in encoders:
                    return (
                        ...
                        [
                            "-b:v",
                            "2000k",  # Even lower bitrate for stability
                            "-maxrate",
                            "3000k",  # Lower max bitrate
                            "-bufsize",
                            "4000k",  # Even smaller buffer
                            "-rc",
                            "cbr",  # Constant bitrate
                            "-rc-lookahead",
                            "0",  # No lookahead for speed
                            "-2pass",
                            "0",  # Disable 2-pass for speed
                        ],
                    )
            ...
            return (
                ...
                [
                    "-crf",
                    "30",  # Even lower quality for max speed
                    "-tune",
                    "zerolatency",  # Zero latency
                    "-x264-params",
                    "ref=1:me=dia:subme=1:analyse=none:trellis=0:no-cabac:aq-mode=0:scenecut=0",
                ],
            )
    ...
    def _high_performance_recording_worker(self):
        ...
        try:
            ...
            final_fps = (
                frames_written / (time.time() - start_time)
                if time.time() > start_time
                else 0
            )
            logger.info(
                f"Recording worker finished: {frames_written} frames, {final_fps:.1f} encoding FPS"
            )
        ...
        if self.ffmpeg_process:
            try:
                ...
                if return_code == 0:
                    ...
                    logger.info(
                        f"Recording completed successfully: {self.output_path} ({file_size:.1f} MB)"
                    )
                    return True
                else:
    ...
    def update_display(self, scale_surface: bool = True):
        ...
        if scale_surface:
            scaled = pygame.transform.scale(
                self.recording_surface, self.screen.get_size()
            )
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    logger.info(
                        f"Recording {'resumed' if self.recording else 'paused'}"
                    )
        return True
    ...
    @abstractmethod
    def render_frame(
        self, surface: pygame.Surface, frame_number: int, dt: float
    ) -> bool:
    ...
    def set_performance_mode(
        self, headless: bool = True, fast: bool = True, use_numpy: bool = True
    ):
        ...
        logger.info(
            f"Performance mode: headless={headless}, fast={fast}, numpy={use_numpy}"
        )
        ...
        try:
            ...
            while not self.is_finished():
                ...
                if not self.render_frame(
                    self.recording_surface, self.current_frame, dt
                ):
                    logger.error(
                        f"Frame rendering failed at frame {self.current_frame}"
                    )
                    break
                ...
                if current_time - last_progress_time >= 5.0:  # Every 5 seconds
                    ...
                    logger.info(
                        f"Progress: {progress:.1f}% ({self.current_frame}/{self.total_frames}) | "
            ...
            if os.path.exists(self.output_path):
                ...
                if file_size > 0.1:  # File has content
                    logger.info(
                        f"Video generated successfully: {self.output_path} ({file_size:.1f} MB)"
                    )
