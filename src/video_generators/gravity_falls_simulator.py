...
class CleanBounce:
    ...
    def update(
        self,
        dt: float,
        container_center: Tuple[float, float],
        ...
        velocity_magnitude = math.sqrt(
            self.vel.vx * self.vel.vx + self.vel.vy * self.vel.vy
        )
...
class GravityFallsSimulator(IVideoGenerator):
    ...
    def initialize_simulation(self) -> bool:
        ...
        try:
            ...
            start_y = center_y + random.uniform(
                -300, -100
            )  # Plus haut pour plus de vitesse

            # Vitesse initiale RAPIDE
            vx = random.uniform(-1000, 1000)  # Beaucoup plus rapide
            vy = random.uniform(-600, 200)  # Plus de variation

            self.ball = CleanBounce(
                pos=Vector2D(start_x, start_y), vel=Velocity(vx, vy), size=15
            )
        ...
        try:
            ...
            if self.ball:
                collision = self.ball.update(
                    dt, self.container_center, self.container_radius, self.container_hue
                )
                if collision:
                    self.bounce_count += 1
            ...
            pygame.draw.circle(
                surface,
                current_container_color,
                self.container_center,
                int(self.container_radius),
                10,
            )
    ...
    def _render_ui(self, surface: pygame.Surface):
        ...
        try:
            ...
            if self.ball and self.ball.size > 80:
                ...
                for dx, dy in [(-2, -2), (-2, 2), (2, -2), (2, 2)]:
                    surface.blit(
                        warning_outline, (warning_rect.x + dx, warning_rect.y + dy)
                    )
