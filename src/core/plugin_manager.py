...
class PluginManager:
    ...
    def register_plugin_dir(self, dir_path: str):
        ...
        path = (
            ...
            else Path(self.base_dir) / dir_path
        )
                ...
                for filename in os.listdir(plugin_dir):
                    if (
                        ...
                        try:
                            ...
                            module_path = (
                                f"{self._package_name(plugin_dir)}.{module_name}"
                            )
                            ...
                            for name, obj in inspect.getmembers(
                                ...
                                if base_class is not None:
                                    if (
                                        ...
                                        discovered[name] = obj
                                        logger.info(
                                            f"Plugin discovered (filtered by {base_class.__name__}): {name}"
                                        )
                                else:
                            ...
                            logger.error(
                                f"Error loading plugin module {module_path}: {e}"
                            )
    ...
    def _organize_by_base_classes(self, plugins: Dict[str, Type]):
        ...
        for plugin_name, plugin_class in plugins.items():
            ...
            for base in plugin_class.__bases__:
                ...
                if (
                    base.__name__.startswith("I")
                    or "base" in base.__name__.lower()
                    or "interface" in base.__name__.lower()
                    or hasattr(base, "__abstractmethods__")
                ):
    ...
    def get_plugin(
        self, name: str, base_class: Optional[Type] = None
    ) -> Optional[Type]:
        """
        ...
        try:
            if base_class is not None:
                ...
                if not self.all_plugins:
                    logger.debug(
                        "Discovering all plugins and organizing by base classes"
                    )
                ...
                for base_name, plugins in self.plugins.items():
                    if name in plugins:
                        logger.debug(
                            f"Plugin found in auto-detected base {base_name}: {name}"
                        )
                        return plugins[name]
...
def create_plugin_manager(
    base_dir: str = "src", plugin_dirs: List[str] = None
) -> PluginManager:
    """
