...
class FFmpegMediaCombiner(IMediaCombiner):
    ...
    def __init__(self):
        ...
        if not self.ffmpeg_path:
            logger.warning(
                "FFmpeg non trouvé, certaines fonctionnalités seront limitées"
            )
    ...
    def combine(
        ...
        try:
            ...
            cmd = [
                ...
                "-i",
                video_path,  # Fichier vidéo
                "-i",
                audio_path,  # Fichier audio
                "-c:v",
                "copy",  # Copier le codec vidéo
                "-c:a",
                "aac",  # Codec audio AAC
                "-b:a",
                "192k",  # Bitrate audio
    ...
    def combine(
        ...
        try:
            ...
            if audio.duration > video.duration:
                logger.info(
                    f"Audio ({audio.duration}s) plus long que la vidéo ({video.duration}s), ajustement..."
                )
