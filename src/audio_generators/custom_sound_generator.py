...
class CustomSoundGenerator:
    ...
    def load_custom_sounds(self):
        ...
        if os.path.exists(config_file):
            ...
            logger.warning(
                "⚠️ Aucun fichier de sons personnalisés trouvé, utilisation des sons par défaut"
            )
    ...
    def generate_sound_from_config(
        self, config: Dict[str, Any], frequency: Optional[float] = None
    ) -> np.ndarray:
    ...
    def generate_sound_by_name(
        self, name: str, frequency: Optional[float] = None
    ) -> Optional[np.ndarray]:
...
class CustomMidiAudioGenerator(IAudioGenerator):
    ...
    def __init__(self, sample_rate: int = 44100):
        ...
        logger.info(
            f"🎵 Custom MIDI Audio Generator initialisé avec {len(self.available_presets)} presets"
        )
    ...
    def configure(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if preset_name and preset_name in self.available_presets:
                ...
                self.current_preset = (
                    random.choice(self.available_presets)
            ...
            logger.info(
                f"Configuration: preset={self.current_preset}, volume={self.volume}"
            )
            return True
    ...
    def apply_trend_data(self, trend_data: TrendData) -> None:
        ...
        if midi_files:
            ...
            if music_path:  # Vérification que music_path n'est pas None
                ...
                logger.warning(
                    "Mélodie par défaut utilisée - sélection musicale échouée"
                )
        else:
            # Fichier MIDI non trouvé, utilise la mélodie par défaut
            self.melody_notes = self.midi_extractor.get_default_melody()
    ...
    def _generate_sound(self, frequency: float, event: AudioEvent) -> np.ndarray:
        ...
        if self.current_preset:
            ...
            if config:
                ...
                sound = self.custom_sound_gen.generate_sound_from_config(
                    config_copy, organic_frequency
                )
                return sound
